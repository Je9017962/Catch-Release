...
function getUsers() {
    try { return JSON.parse(localStorage.getItem("car_users") || "{}"); } catch { return {}; }
}
...
function getSession() {
    try { return JSON.parse(localStorage.getItem("car_session") || "null"); } catch { return null; }
}

function saveSession(email) {
    localStorage.setItem("car_session", JSON.stringify({ email: email, ts: Date.now() }));
}
...
function withLoading(btn_id, fn) {
...
function doSignup() {
    withLoading("signup-btn", async () => {
        ...
        users[email] = { name: name, email: email, hash: hash, createdAt: Date.now(), onboarded: false, totalXP: 0, catchLog: [] };
        saveUsers(users);
        saveSession(email);
...
function doReset() {
    withLoading("reset-btn", async () => {
        ...
        ["reset-email", "reset-pass1", "reset-pass2"].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = "";
        });
...
function enterApp() {
    ...
    const username_el = document.getElementById("profile-username");
    if (username_el) username_el.textContent = user.name;
