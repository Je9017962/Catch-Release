...
const WMO = {
  ...
  3:{icon:"☁️",desc:"Overcast"},45:{icon:"🌫️",desc:"Foggy"},48:{icon:"🌫️",desc:"Icy Fog"},
  ...
  95:{icon:"⛈️",desc:"Thunderstorm"},96:{icon:"⛈️",desc:"Thunderstorm + Hail"},99:{icon:"⛈️",desc:"Heavy Thunderstorm"},
};

function wmoLookup(code) { return WMO[code] || { icon:"🌡️", desc:"Unknown" }; }
...
async function fetchWeather() {
  ...
  if (!city) {
    try {
      ...
      if (r.ok) { const j = await r.json(); city = j.city || j.locality || j.countryName || ""; }
    } catch { }
  }
  ...
  try {
    ...
    const temp  = city ? `${tempF}°F · ${wmo.desc} · ${city}` : `${tempF}°F · ${wmo.desc}`;
    applyWeatherUI(wmo.icon, date, temp);
    window._weatherCache = { icon: wmo.icon, date, temp };
  } catch (e) {
...
function refreshARWeather() {
  ...
  if (!c) return;
  ["ar-weather-icon","ar-weather-date","ar-weather-temp"].forEach((id,i) => {
    const el = document.getElementById(id);
    if (el) el.textContent = [c.icon, c.date, c.temp][i];
  });
}
